...
def create_batches(documents: List[Document], batch_size: int = 50) -> List[List[Document]]:
    ...
    # put the documents into batches to be sent to vector database
...
async def upload_batch(batch: List[Document], batch_num: int) -> bool:
    ...
    # try to upload batch to firebase asynchronously
    try:
        await vectorstore.aadd_documents(batch)
        print(f"Batch {batch_num} successfully uploaded.")
        return True
    # otherwise throw an error saying failed to upload batch
...
async def main():
    ...
    loader = UnstructuredLoader(file_path=f"{cwd}", chunking_strategy="basic", max_characters=10000000)
    ...
    # loop through the split docs
    for doc in doc_splits:
        # Retain only necessary fields to avoid exceeding Pinecone's 40KB limit
        source = doc.metadata.get("source") or doc.metadata.get("url") or "unknown"
        doc.metadata = {"source": source}

    # create batches to be sent to the pinceone database
